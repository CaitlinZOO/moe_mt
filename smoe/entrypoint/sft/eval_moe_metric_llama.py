...
def tokenize_text(text, lang):
    ...
    if lang in ['ar', 'tr', 'hi', 'ru', 'es', 'fr', 'de', 'it', 'en', 'pt', 'cs', 'et', 'fi']:  # 阿拉伯语、土耳其语、印地语、俄语、西班牙语、法语、德语、意大利语、英语
                ...
                # if file in ['ECOIT.test.jsonl','IIMT.test.jsonl','OCRMT30K.test.jsonl','Dota.test.jsonl']:
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        for line in f.readlines():
                            ...
                            # 使用正则表达式匹配"-"前后的字符串
                            match = re.match(r'([^-]*)-([^-]*)', item['filename'])
                            # # 如果匹配成功，match对象将包含两个分组，分别对应"-"前后的字符串
                            if match:
                                 before_dash = match.group(1)  # "-"前面的字符串
                                 after_dash = match.group(2)   # "-"后面的字符串
                                 print(f"Before dash: {before_dash}, After dash: {after_dash}")
                                 tgt_lang = after_dash
                            else:
                            ...
                            bleu, meteor  = calculate_metrics(reference, hypothesis, tgt_lang=tgt_lang)
